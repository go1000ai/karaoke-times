        ...
        <section className="px-5 space-y-3">
          {allVenues.map((venue) => (
            <Link
              key={venue.id}
              href={`/venue/${venue.id}`}
              className="flex gap-4 bg-white p-4 rounded-2xl items-center shadow-sm border border-border"
            >
              ...
              <div className="flex-grow min-w-0">
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-gold text-xs font-bold flex items-center gap-0.5">
              ...
              <span className="material-icons-round text-crimson">favorite</span>
            </Link>
          ))}
        </section>
      ) : (
